...
type NotificationEvent = keyof EventData | "open" | "close" | "error";

/**
 * Library independent connection interface.
 * Can be used to proxy calls to different libraries or through intermediate APIs.
 */
export default abstract class Connection extends EventEmitter<NotificationEvent> {
    /**
    ...
    emit<T extends EventEmitter.EventNames<NotificationEvent>>(
        event: T,
        ...args: EventEmitter.EventArgs<NotificationEvent, T>
    ): boolean {
        ...
        if (event.startsWith(LEGACY_NOTIFICATION_PREFIX)) {
            if (super.emit(
                ...
                ...args as EventEmitter.EventArgs<NotificationEvent, T>
            )) {
                result = true;
            }
        }
