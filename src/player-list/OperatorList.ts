...
export default class OperatorList extends PlayerList<Operator, Operator, Player> {
    ...
    protected parseResult(result: unknown[]): Operator[] {
        ...
        for (const [index, entry] of result.entries()) {
            ...
            const operator = new Operator(Player.parse(entry.player, index + ".player", result));
            if ("permissionLevel" in entry) {
                if (typeof entry.permissionLevel !== 'number') {
                    throw new IncorrectTypeError("number", typeof entry.permissionLevel, result, `${index}.permissionLevel`);
                }
            ...
            if ("bypassesPlayerLimit" in entry) {
                if (typeof entry.bypassesPlayerLimit !== 'boolean') {
                    throw new IncorrectTypeError("boolean", typeof entry.bypassesPlayerLimit, result, `${index}.bypassesPlayerLimit`);
                }
